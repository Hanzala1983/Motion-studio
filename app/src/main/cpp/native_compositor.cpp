    ...
    for (const auto& layer : layers) {
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                ...
                float srcA = (sample[3] / 255.0f) * layer.opacity;
                if (srcA <= 0.0f) continue;

                uint8_t* dst = outRgba + (y * outWidth + x) * 4;
                // Standard "over" alpha compositing, straight alpha.
                for (int c = 0; c < 3; c++) {
                    dst[c] = (uint8_t) std::clamp(sample[c] * srcA + dst[c] * (1.0f - srcA), 0.0f, 255.0f);
                }
